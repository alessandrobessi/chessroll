...
| `replay`    | PGN   | ✅ done | Full game replay, importance-weighted per move (quiet 0.35s → capture 0.50s → check 0.60s → large eval swing 0.90s+pause → critical move 1.20s+pause+`!!`/`??` annotation). Persistent player name/rating labels flush to the board's own left edge (whoever's on top/bottom follows orientation), event/year subtitle, running move number, optional evaluation, the game's actual result at the end.                                                                    |
